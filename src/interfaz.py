...
class InfoLabel(Box):
    ...
    def create_my_box(self, manager):
        self.label = pyg.elements.UILabel(relative_rect=self.true_rect, text = f"{self.name} = {str(self.obj)}",manager = manager)
...
class DeleteBoton(Boton):
    def __init__(self, size, pos, name, command):
        super().__init__(size, pos, "Delete", command)
        self.name = name
...
class Slider:
    def __init__(self, size, pos, minimo, maximo):
    ...
    def click(self, pressed, mouse, pos):
        if self.true_rect.collidepoint(pos) and pressed[0]:
            if mouse[0] > 0:
                self.move(0.02)
            if mouse[0] < 0:
                self.move(-0.02)
